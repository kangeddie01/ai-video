...
def _configure_google_credentials():
    credential_path = os.getenv("GOOGLE_KEY_FILE_PATH") or os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
...
def make_voice_google(text, model_name=None, file_name=None):
...
if __name__ == "__main__":
    ...
    for index, text in enumerate(lines, start=1):
        ...
        file_name = f"{book}_{chapter}_{verse_no}_{model.strip()}.mp3"
        # output_path = f"output/audio/bible/{file_name}"
        # print(output_path)
        res = make_voice_google(verse_text,  "ko-KR-Chirp3-HD-" + model.strip(), file_name)        
