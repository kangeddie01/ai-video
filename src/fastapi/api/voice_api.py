...
router = APIRouter(prefix="/generate", tags=["Voice"])

# fastapi_domain="http://127.0.0.1:8000"
down_path = "download"
static_root_dir = "data"
...
async def create_voice(request: Request, body: VoiceRequest):
    ...
    if body.vendor == "openai":
        ...
        res = make_voice_openai(
            text=body.text, instructions=body.instructions, voice=body.voice
        )
    ...
    return VoiceResponse(success=True, voice_path=return_url)
