...
def parse_verse_line(line: str):
    ...
    if not match:
        return None, line.replace(",", "")

    verse_no = int(match.group(1))
    verse_text = match.group(2).replace(",", "").strip()
...
def make_voice_google_with_retry(
    ...
    raise RuntimeError(f"Google TTS 생성 최종 실패: {output_path}, error={last_error}")
...
def make_voice_google(
    ...
    voice = texttospeech.VoiceSelectionParams(
        language_code=lang_cd, name=model_name if model_name else None
    )
...
def make_silent_mp3(
    output_path: str | Path, duration: float, force_recreate: bool = False
):
    """
...
def build_audio_list_with_pause(
    source_audio_files: list[str], silence_path: str, add_last_pause: bool = True
) -> list[str]:
    """
...
def concat_mp3_files(audio_files: list[str], output_path: str | Path):
    """
...
def create_chapter_voice_package(
    ...
    model = model.strip()
    google_voice_name = f"{lang}-Chirp3-HD-{model}"
    ...
    chapter_dir = Path(create_default_path) / book_code / chapter_str
    print("chapter_dir : " + str(chapter_dir))
    ...
    for line_index, line in enumerate(lines, start=1):
        ...
        verse_str = f"{verse_no:02d}"

        source_audio_path = (
            chapter_dir / f"{book_code}_{chapter_str}_{verse_str}_{lang}_{model}.mp3"
        )
    ...
    if pause > 0:
        ...
        silence_path = Path(create_default_path) / silence_name

        make_silent_mp3(output_path=silence_path, duration=pause)
    ...
    concat_mp3_files(audio_files=concat_files, output_path=body_audio_path)
    ...
    result = {
        ...
        "silence_audio": (
            str(silence_path).replace("\\", "/") if silence_path is not None else None
        ),
    ...
    return {
        ...
        "segment_count": len(segments),
        "chapter_txt": f"Chapter {chapter}" if lang == "en-GB" else f"{chapter}장",
    }
...
def create_book_voice_packages(
    ...
    summary = []
    to_chapter = end_chapter if end_chapter > 0 else chapter_count + 1
    for chapter in range(start_chapter, to_chapter + 1):
    ...
    summary_path = (
        ...
        / book_code
        / f"{book_code}_voice_package_summary.json"
    )
...
if __name__ == "__main__":
    ...
    lang = "en-GB"
    path = Path("data/bible/openbible/book_list.json")
    ...
    for index, book_info in enumerate(book_list, start=1):
        ...
        book_name_en = book_info.get("bookNmEn2")
        book_name_ko = book_info.get("bookNmEn1")  # 영어일때만, 한글일때 bookNmKo
        ...
        create_book_voice_packages(
            ...
            chapter_count=chapter_count,
            input_pattern=(
                f"data/bible/openbible/{lang}/{book_no:02d}.{book_name_en}/"
                f"{book_name_en}-{{chapter:02d}}.txt"
            ),
            model="sadachbia",
            lang=lang,
            create_default_path=f"data/bible/audio/{lang}",
            pause=0.4,
